...
namespace JusticeFramework.Collections {
	...
	public class Inventory {
		...
		/// </summary>
		[SerializeField]
		private List<InventoryEntry> contents = new List<InventoryEntry>();
		...
		public bool Contains(string id, int minimumQuantity) {
            int index = IndexOf(id);
            return index != -1 && contents[index].Quantity >= minimumQuantity;
        }

        /// <summary>
        /// Determines if the list contains an item with the given tag.
        /// </summary>
        /// <param name="tag">The tag to search for</param>
        /// <param name="quantity">The minimum stack quantity needed for the item to pass</param>
        /// <returns>Returns true if there exists an item with the specified tag, false otherwise.</returns>
        public bool HasItemWithTag(string tag, int quantity = 1) {
            return GetFirstItemWithTag(tag, quantity) != null;
        }

        /// <summary>
        /// Gets the first item in the list that matches the given tag.
        /// </summary>
        /// <param name="tag">The tag to search for</param>
        /// <param name="quantity">The minimum stack quantity needed for the item to pass</param>
        /// <returns>Returns an ItemData record for the first item with the tag, null otherwise.</returns>
        public ItemData GetFirstItemWithTag(string tag, int quantity = 1) {
            ItemData data = null;
            ItemData temp;

            for (int i = 0; i < contents.Count && data == null; i++) {
                temp = GameManager.DataManager.GetAssetById<ItemData>(contents[i].Id);

                if (GameTag.HasTag(temp, tag) && contents[i].Quantity >= quantity) {
                    data = temp;
                }
            }
        ...
        public int GetQuantity(string id) {
            int itemIndex = IndexOf(id);
            return itemIndex != -1 ? contents[itemIndex].Quantity : 0;
        }
